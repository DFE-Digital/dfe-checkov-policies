...
| DFE_ELZ_0006 | Enforce write-only network security groups on subnets               | Ensure that subnets are using write-only network security groups, which also means not allowing bypassing via an association resource                                                | Network    | Active |
